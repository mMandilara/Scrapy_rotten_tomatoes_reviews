...
from ..items import LeastRatedMoviesItem
import os
import json

# !To run this script go to path - where least_rated_movies folser is - and run in command window 'scrapy crawl bad_movies'!
class MoviesSpider(scrapy.Spider):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self.item_count = 1
    self.max_item_total = 100
    self.output_file = 'bad_reviews.json'

    self.start_urls = [f'https://www.rottentomatoes.com/browse/movies_at_home/sort:critic_lowest?page={page}' for page in range(1, 3)]
